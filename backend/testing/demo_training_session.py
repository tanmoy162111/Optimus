"""
Demo Training Session - Quick demonstration of key tools
"""
import sys
import os
...
def demo_training_session():
    """Demo training session with key tools"""
    ...
    key_tools = ['nmap', 'whatweb', 'nikto']
    
    for target in targets[:2]:  # Start with first 2 targets
        print(f"\n{'='*80}")
        print(f"Demo Training Session: {target['name']}")
        ...
        for tool_name in key_tools:
            try:
                ...
                tool_result = tool_manager.execute_tool(
                    ...
                    scan_id=f"demo_{tool_name}_{int(time.time())}",
        ...
        # Save results
        filename = os.path.join(os.path.dirname(__file__), 'data', f"demo_session_{target['name'].replace(' ', '_')}.json")
    ...
    print("\n✅ Demo Training Session Complete")
