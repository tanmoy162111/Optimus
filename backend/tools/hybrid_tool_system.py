...
class LLMCommandGenerator:
    """LLM-powered command generation with structured output"""
    
    def __init__(self, llm_client):
        self.llm_client = llm_client
        # Import the structured command class from the main LLM command generator
        try:
            from .llm_command_generator import GeneratedCommand as StructuredGeneratedCommand
            self.structured_command_cls = StructuredGeneratedCommand
        except ImportError:
            # Fallback implementation if main generator is not available
            from dataclasses import dataclass
            from enum import Enum
            ...
            @dataclass
            class FallbackGeneratedCommand:
                command: str
                explanation: str
            ...
            self.structured_command_cls = FallbackGeneratedCommand
    
    def generate_command(self, tool_name: str, task: str, target: str, 
                        help_text: Optional[str] = None):
        """Generate command using LLM with structured output"""
        ...
        return self.structured_command_cls(
            command=command,
            explanation=f"Generated by LLM for task: {task}",
            safety_level=self.structured_command_cls.__annotations__.get('safety_level', type('Enum', (), {'value': 'medium'}))(),
            confidence=0.75,
            requires_root=False,
            warnings=["LLM-generated command - review before execution"],
            alternatives=[],
            tool=tool_name,
            arguments=[target],
            target=target
